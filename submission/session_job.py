...
GEOCODING_API_KEY = "FFDF639F9892C076603667C6A4CE26B4"
...
group_by_df = (
    ...
    .select(
        sha2(
            concat_ws("_", col("user_id"), col("ip"), col("session_window.start")), 256
        ).alias("session_id"),
        col("session_window.start").alias("session_window_start"),
        col("session_window.end").alias("session_window_end"),
        date_format(col("session_window.start"), "yyyyMMdd")
        .cast("int")
        .alias("session_date"),
        col("country"),
        col("state"),
        ...
        col("os"),
        col("browser"),
        when(col("user_id").isNull(), False).otherwise(True).alias("is_user_logged"),
        col("count").alias("event_count"),
    )
)

query = (
    group_by_df.writeStream.format("iceberg")
    .outputMode("append")
