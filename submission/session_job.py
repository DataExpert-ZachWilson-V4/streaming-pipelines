...
GEOCODING_API_KEY = '6345B50C4212136B2796EF976D30EDCD'
...
tumbling_window_df = kafka_df \
    ...
    .withWatermark("timestamp", "30 seconds") # wait 30mins at end of window



def generate_session_id(user_id, ip, window_start):
    """
    Generate a session ID hash from the given user ID, IP address, and window start time.
    Parameters:
        user_id (int): The ID of the user.
        ip (str): The IP address.
        window_start (int): The start time of the window.
    Returns:
        int: The session ID hash.
    """
    return hash(str(user_id) + ip + str(window_start))

generate_session_id_udf = udf(generate_session_id, IntegerType())

by_session = (tumbling_window_df.groupBy(window(col("timestamp"), "5 minutes"),
                       col("value.user_id"),
                       col("value.ip"),
                       col("geodata.country"),
                       col("geodata.city"),
