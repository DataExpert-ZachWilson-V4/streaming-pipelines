...
# Function to geocode IP addresses
def geocode_ip_address(ip_address):
    url = GEOCODING_API_URL
    try:
        response = requests.get(
            url, params={"ip": ip_address, "key": GEOCODING_API_KEY}
        )
...
# Load Kafka configuration from arguments
...
# Define the schema for geocoded data
...
def aggregate_sessions(event_df):
    # Generate session IDs by hashing user_id, ip, and event_time
    session_id_udf = udf(
        lambda user_id, ip, event_time: hash(user_id + ip + str(event_time)),
        StringType(),
    )

    return (
        event_df.withWatermark("event_time", "10 minutes")
        .groupBy(
            session_window(col("event_time"), "5 minutes"), col("user_id"), col("ip")
        )
        .agg(
        ...
        .select(
            session_id_udf(col("user_id"), col("ip"), col("window.start")).alias(
                "session_id"
            ),
            col("user_id"),
            col("ip"),
            ...
            col("event_count"),
            when(col("user_id").isNotNull(), True)
            .otherwise(False)
            .alias("is_logged_in"),
        )
    )
...
# Write the aggregated data to an Iceberg table
...
def main():
    ...
    try:
        ...
        query = write_to_iceberg(
            session_df, args["output_table"], args["checkpoint_location"]
        )
