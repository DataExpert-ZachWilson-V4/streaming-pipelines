...
from pyspark.sql.functions import col, lit, from_json, udf, session_window, when
...
GEOCODING_API_KEY = 'FD15A6B495BA579B54B7A675800F887C'
...
# creating session id from user_id, ip and timestamp
def create_session(user_id, ip, window_start):
    return hash(str(user_id) + ip + str(window_start))    

session_hash = udf(create_session, StringType())
...
by_sessions = tumbling_window_df.groupBy(session_window(col("timestamp"), "5 minutes"),
                                        col("value.user_id"),
                                        col("value.ip"), 
    ...
    .count() \
    .select(
        session_hash(col("user_id"), col("ip"), col("session_window.start")).alias("session_id"),
        col("user_id"),
        col("ip"),
        col("country"),
        col("state"),
        ...
        .otherwise("user_logged_out").alias("user_session_status")
